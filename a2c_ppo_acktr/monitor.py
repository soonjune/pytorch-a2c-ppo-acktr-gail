...
class RepeatMonitor(gym.Wrapper):
    """
    ...
    def __init__(
        ...
        super(RepeatMonitor, self).__init__(env=env)
    ...
    def step(self, action: Union[np.ndarray, int]) -> GymStepReturn:
        ...
        observation, reward, done, info = self.env.step(action, repeat)
...
class ResultsWriter:
    ...
    def __init__(
        ...
        if header is None:
            header = {}
        if not filename.endswith(RepeatMonitor.EXT):
            if os.path.isdir(filename):
                filename = os.path.join(filename, RepeatMonitor.EXT)
            else:
                filename = filename + "." + RepeatMonitor.EXT
